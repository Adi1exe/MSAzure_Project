        ...
        function handleFile(file) {
            const allowedTypes = ['audio/wav', 'audio/mpeg', 'audio/mp3', 'audio/ogg', 'audio/m4a', 'audio/x-m4a', 'audio/flac'];
            const fileExtension = file.name.split('.').pop().toLowerCase();
            const allowedExtensions = ['wav', 'mp3', 'ogg', 'm4a', 'flac'];
            
            if (!allowedExtensions.includes(fileExtension)) {
                const statusDiv = document.getElementById('stt-status');
                showStatus(statusDiv, 'Invalid file type. Please upload WAV, MP3, OGG, M4A, or FLAC files.', 'error');
                return;
            }
